    ...
    def add_vertex(self, val):
        ...
        # Só cria se o vértice não existir ainda
    ...
    def add_edge(self, origin, dest, rel = None):
        ...
        # Se o grafo não for direcionado, adicionamos a relação no sentido oposto, isto é, destino->origin
    ...
    def __degree(self, vertex):
        """ Se vertex existir no dicionário self.__data, retorna o número de elementos do respectivo conjunto de vértices adjacentes"""
        if vertex in self.__data: return len(self.__data[vertex])
        else: return 0

    """
        Método que ccerifica se um dererminado vérice é referenciado por outro (necessário na exclusão de vértices de grafos direcionados)
    """
    def __is_referenced(self, vertex):
        for vtx_ref in self.__data.keys():  # Percorrendo o dicionário
            for edge in self.__data[vtx_ref]: return True
        return False
