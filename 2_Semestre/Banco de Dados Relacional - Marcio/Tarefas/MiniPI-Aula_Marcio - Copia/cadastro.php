...
if ($received_data->action == 'insert') { // Insere um novo aluno no banco, passando primeiro e segundo nome
    $data = array(
...
if ($received_data->action == 'fetchSingle') { // Busca por id dos alunos
    $query = "
 SELECT * FROM fatec_alunos 
...
if ($received_data->action == 'delete') { // Função para deletar um registro de admin do banco
    $query = "
 DELETE FROM fatec_admin 
 WHERE id = '" . $received_data->id . "'
 ";
